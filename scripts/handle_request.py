...
class HandleRequest:
    ...
    def do_request(self, url, method="post", data=None, is_json=False, head=None):
        """
        发起请求方法
        #is_json为Ture就以json形式传参，  为False就以www.from表单传参
        # data = '{"name": "阿登"}'     # json格式的字符串
        # data = "{'name': '阿登'}"     # 字典类型的字符串
        :param url:地址
        :param method: 请求方法
        :param data: 请求数据
...
#     shop_data = {"id":None,"name":"都是发是的","desc":"这是一个描述","code":"znjf","exhibitionChannel":"1"}
